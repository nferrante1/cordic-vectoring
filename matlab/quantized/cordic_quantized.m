% Cordic quantized
% This script must be run after float/cordic_float.m
...
steps = 14;
% We choose 14 because the elementary angles, when quantized, are
% always 0 after the 14th value (2^-13 --> 1, 2^-14 --> 0). So
% with 16 bit inputs 14 steps of cordic are sufficient
%-----------------------------------------------------------

%-------------Generate input values-------------------------
% Generate input as points lying on circles with different
% radius
theta = (-pi:0.25:pi);
r = (0.1:0.005:0.9);
...
% Write angles to file (we need to do this here since
% this file will be read again from function
% cordic_vectoring_q)
...
%-------------Check results---------------------------------
% Output of arrayfun is a cell and must be converted to
% matrix
...
% Compute expected values for phase and radius using arctan
% for phase and sqrt(x^2 + y^2) for radius
...
% Compute mean square error on phase and radius with the
% float model
